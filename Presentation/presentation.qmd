...
-   I want to learn how a min-heap works
    -   Specifically, I am interested in mechanism for popping the minimum element
-   I am a visual learner -- seeing images of popping an element would be helpful
-   Utilizing ChatGPT 4o
...
![](img5.webp){fig-align="center"}

## Takeaways

-   ChatGPT was not able to successfully complete the task
-   Even after simplifying the task, ChatGPT was unable to generate a simple binary tree.
-   The process was very iterative.
  - Each step was either reattempting the previous step, or simplifying the task.

# Success

## Background

-   I want to create a Streamlit frontend application to view historical stock data
-   Specifically, I want to show historical earnings dates and options data
...
Next, I want to display the some earnings history data. Particularly, I want to display the "reportedDate" and "reportedTime" (before or after market) for previous X earnings. X should be an input that the user can enter.

DataRetriever has a method called "getEarningsHistory" which takes the number of earnings (integer) to show as an input. Thus, pass X as an integer as an input to that method. "getEarningsHistory" will return a list of tuples, containing pairs of (reportedDate, reportedTime). I want to display this as a table on the application, where the column names are "reportedDate" and "reportedTime". Again, I want to ensure that if there is no ticker inputted, then there should be no field to enter X and no earnings history data shown on the table.
...
-   Instead of including all instructions at once, I gave ChatGPT instructions one after another, such that the solutions built on top of one another -- piece by piece.
    -   This is to prevent recency bias as too many instructions at once may lead to ChatGPT prioritizing the recent ones.
-   The process was very iterative, with each step having very specific instructions.

## Conclusion

- ChatGPT was not able to generate a simple binary tree, let alone a min-heap.
  - It kept generating structures that either resembled real trees (trees in nature) or graphs, but not the tree data structure, even after specifying properties of the tree data structure.
  - I hypothesize that this is due to:
    - Small technical training data for image generation.
    - Images are higher dimensional. Thus, ChatGPT may be struggling to interpret the image it generated.
    - ChatGPT combined elements of real-world with CS elements to generate some abstract representation.
      - Trees in nature with the trees data structure.
      - Children (kids) with child nodes in the tree data structure.
- ChatGPT was successfully able to write the Python code to generate the frontend for my Streamlit application.
  - It did so without any errors.
  - I hypothesize that this is due to:
    - Large training data for code generation.
    - Specific and step-by-step instructions were provided.
    - The task was relatively straightforward.

### Learnings
  - ChatGPT is really good at writing code. However, it struggles to generate images with exact specifications. It may interpret prompts differently than humans, combining elements to generate an abstract image.
  - A tool designed for diagram generation, such as Graphviz, draw.io, and NetworkX, may be better suited.
  - Specificity is important. I was able to achieve the best results, for both the challenge and success, when my prompts were specific. Being broad led to worse results.
  - The process is very iterative, as ChatGPT is able to build on a solution one step at a time.
